...
@export_range(1, 4) var mip_level:int = 1 #how much to scale the screen by
...
func initialize_compute_shader() -> void:
	...
	if rd:
		...
		var glsl_file:RDShaderFile = load("res://compositor_effects/BlurEffect.glsl")
...
func do_pass(render_data: RenderData, pass_num:int) -> void:
	...
	#shader workgroup sizes. '16' correlates with local size in shader and was chosen arbitrarily
	...
	#check backbuffers. Create new ones if view size or count changed
	...
	packed_ints.append(min(blur_samples, blur_width)) #make sure not to sample more than the kernel size
	...
	packed_bytes.resize(32) #resize and add enough padding for nearest 16 byte interval
	...
	for view in view_count:
		...
		if pass_num == 1:
			...
			uniform.binding = 0
			uniform.add_id(backbuffers[view]) #buffer1
			...
			uniform.uniform_type = RenderingDevice.UNIFORM_TYPE_IMAGE
			uniform.binding = 0
			uniform.add_id(backbuffers[view_count + view]) #buffer2
